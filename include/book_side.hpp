...
public:
        ...
        inline bool add(Price price, Qty qty) noexcept {
		if(!is_valid_price(price)) return false;
        ...
        inline bool cancel(Price price, Qty qty) noexcept {
		if(!is_valid_price(price)) return false;		

		const size_t idx = index_of(price);
		auto& level = levels[idx];

		if(level.volume < qty) return false;
		level.volume -= qty;
